...
describe('tests game component', () => {
    ...
    test('renders game status correctly', () => {
        ...
        const firstPlayer = wrapper.find('.game .game-info').children().first().text();
        expect(firstPlayer).toEqual('Next player: X');

        // player 1
        const button = wrapper.find('button.square').first()
        button.simulate('click');
        const secondPlayer = wrapper.find('.game .game-info').children().first().text();
